...
function apply(ctx) {
  ...
  let seq = 0;
  let owner = undefined; // 当前 agent（jobs.read/list 的 caller），不序列化
  ...
  if (jobs) {
    const offChanged = jobs.onJobsChanged((own) => {
      try {
        ...
        for (const snap of list) syncJob(snap);
    ...
    try {
      if (owner) for (const snap of jobs.list(owner)) syncJob(snap);
    } catch (e) { /* noop */ }
  }
  ...
  if (timer) {
    const offPoll = timer.interval(() => {
      try {
        if (!streamEnabled || !jobs || !owner) return;
      ...
      } catch (e) { console.error('cmdmon poll failed', e); }
    }, 500);
    ctx.effect(() => offPoll);
  }
