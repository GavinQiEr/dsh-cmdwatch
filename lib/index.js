...
function apply(ctx) {
  ...
  if (subprocess && typeof subprocess.spawn === 'function') {
    ...
    subprocess.spawn = (spec) => {
      ...
      if (matchedCallId === undefined) return handle;
    ...
    function matchesCommand(spec, command) {
      ...
      return tail.includes(command) || (tail.length > 0 && command.includes(tail.trim()));
    }
    ctx.effect(() => {
      // 恢复原生 spawn（插件卸载时）
  ...
  // 增量快照：GET /cmdmon/snapshot?since=N
