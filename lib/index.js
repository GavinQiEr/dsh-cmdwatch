...
const inject = ['webServer', 'shell', 'subprocess'];
...
function apply(ctx) {
  ...
  const streamCandidates = new Map(); // callId -> command（待认领的前台流式候选）
  const streamQueue = []; // FIFO：tools/execute 登记的前台 pwsh 候选（顺序保证匹配）
  let activeStreamCallId = undefined; // 当前正在流式推送的 tool 记录 key
  const streamingCalls = new Set(); // 已进入流式镜像的 callId（tools/result 跳过重复追加）
  const streamedCalls = new Set(); // 已完成流式推送的 callId（保留用于清理）
  let seq = 0;
  ...
  ctx.on('tools/execute', (exec, next) => {
    try {
      ...
      touch(r);
      // 前台 pwsh/bash 命令（未走后台任务）登记为流式候选：
      // subprocess.spawn 包装按 FIFO 匹配认领，实现执行期间的增量推送
      const args = exec && exec.arguments;
      const isForeground = !(args && typeof args === 'object' && args.run_in_background === true);
      if ((toolName === 'pwsh' || toolName === 'bash') && isForeground && cmd) {
        streamCandidates.set(String(exec.callId), cmd);
        streamQueue.push(String(exec.callId));
      }
  ...
  ctx.on('tools/result', (exec, result) => {
    try {
      ...
      // 流式已推送过完整输出（含 [stderr] 段），tools/result 只更新状态、不重复追加
      if (streamingCalls.has(String(exec.callId))) {
        streamingCalls.delete(String(exec.callId));
        streamedCalls.delete(String(exec.callId));
        touch(r);
        return;
      }
  ...
  // ---- 前台命令流式化：包装 subprocess.spawn ----
  // 机制：前台 pwsh/bash 的 shell.run 同步等待、中间输出不可见。所有 shell 执行
  // （run 和 start）最终都走 ctx.subprocess.spawn。spawn 返回的 handle.collected
  // 是 offset-based 非消耗式 reader（"independent readers never consume one
  // another's output"），因此可以并行镜像读取而不影响工具自身的读取。
  // 我们不替换执行、不改超时/sandbox/退出码语义——只是旁观并推送增量。
  if (subprocess && typeof subprocess.spawn === 'function') {
    console.error('cmdmon debug: subprocess.spawn wrapper installed');
    const nativeSpawn = subprocess.spawn.bind(subprocess);
    subprocess.spawn = (spec) => {
      const joined = spec && spec.argv && Array.isArray(spec.argv) ? spec.argv.join(' ') : '';
      console.error('cmdmon debug: spawn called queue=' + streamQueue.length + ' argvHead=' + JSON.stringify(joined.slice(0, 80)));
      const handle = nativeSpawn(spec);
      // FIFO 匹配：取队首候选；若命令不吻合则跳过（保留给下一个 spawn）
      let matchedCallId = undefined;
      while (streamQueue.length > 0 && matchedCallId === undefined) {
        const candidateCallId = streamQueue[0];
        const candidateCmd = streamCandidates.get(candidateCallId);
        if (candidateCmd !== undefined && matchesCommand(spec, candidateCmd)) {
          matchedCallId = candidateCallId;
          streamQueue.shift();
        } else {
          // 命令不匹配：该候选可能已过时（tools/result 已处理），丢弃
          streamQueue.shift();
          streamCandidates.delete(candidateCallId);
        }
      }
      if (matchedCallId === undefined) {
        if (streamQueue.length > 0) {
          console.error('cmdmon debug: spawn no-match, queue=' + streamQueue.length + ' argvTail=' + JSON.stringify(spec && spec.argv ? spec.argv.slice(-2).join(' ').slice(0, 60) : ''));
        }
        return handle;
      }

      streamCandidates.delete(matchedCallId);
      activeStreamCallId = matchedCallId;
      streamingCalls.add(matchedCallId);
      const target = records.get('tool:' + matchedCallId);
      const stdoutReader = handle.collected && handle.collected.stdout;
      const stderrReader = handle.collected && handle.collected.stderr;
      if (!target || !stdoutReader) return handle;

      // 增量镜像：并行读取 collected（独立 cursor，不影响工具读取）
      let outOffset = 0;
      let errOffset = 0;
      const poll = () => {
        try {
          const out = stdoutReader.readFrom(outOffset);
          outOffset = out.nextOffset;
          let text = out.text;
          if (stderrReader) {
            const err = stderrReader.readFrom(errOffset);
          ...
          if (text && target) appendOutput(target, text);
        } catch (e) { /* reader 可能已随进程结束 */ }
      };
      const offPoll = timer && typeof timer.interval === 'function'
        ? timer.interval(poll, 300)
        : undefined;
      // 进程结束收尾：清 interval + 最后一次读取 + 标记流式完成
      handle.done.then(() => {
        if (offPoll) offPoll();
        poll();
        if (activeStreamCallId === matchedCallId) activeStreamCallId = undefined;
        // 不在此删除 streamingCalls：tools/result 负责消费（保证其触发时仍能判断）
        // 若 tools/result 因异常未触发，streamedCalls 用于延迟清理
        streamedCalls.add(matchedCallId);
      }, () => {
        if (offPoll) offPoll();
        if (activeStreamCallId === matchedCallId) activeStreamCallId = undefined;
        // spawn 失败：不标记 streamed，让 tools/result 正常追加错误
        streamingCalls.delete(matchedCallId);
      });
      return handle;
    };
    // 匹配辅助：spawn spec 的 argv[0] 是解释器；命令体出现在 argv 尾部
    // （pwsh: -Command <cmd>；bash: -c <cmd>）。用"命令字符串是否出现在 argv 中"
    // 作为宽松匹配，避免精确比较在编码/换行上的差异。
    function matchesCommand(spec, command) {
      if (!spec || !spec.argv || !Array.isArray(spec.argv) || !command || command.length < 8) return false;
      // 检查整个 argv 拼接（sandbox 的 confine 可能把命令体放在 argv 任意位置）
      const joined = spec.argv.join(' ');
      return joined.includes(command);
    }
    ctx.effect(() => {
      // 恢复原生 spawn（插件卸载时）
      subprocess.spawn = nativeSpawn;
    });
  }
