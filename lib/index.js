...
import { analyzeCommand, buildForegroundTee } from './rewrite.js';
...
function apply(ctx, config) {
  ...
  ctx.on('tools/execute', (exec, next) => {
    try {
      ...
      if (args && typeof args.command === 'string') {
        ...
        // 诊断：宿主控制台输出净化决策，便于排查"没改写/没流"（debug 开启时）
        if (meta.changed || meta.warnings.length || bg) {
          dbg(`[cmdmon] execute tool=${toolName} bg=${bg} changed=${meta.changed} warnings=${meta.warnings.length}`);
          if (meta.changed) dbg(`[cmdmon] rewrite:\n  ${JSON.stringify(meta.originalCommand)}\n  -> ${JSON.stringify(cmd)}`);
        }
        // 前台命令实时输出：插入 Tee 落盘，面板轮询文件显示进度（默认开启）
        if (!bg && cfg.foregroundStream && (toolName === 'pwsh' || toolName === 'bash')) {
          try {
            const wrap = buildForegroundTee(args.command, {
              tool: toolName,
              callId: exec && exec.callId,
              workdir: typeof args.workdir === 'string' ? args.workdir : undefined
            });
            if (wrap) {
              exec.arguments = { ...args, command: wrap.wrapped };
              meta.fgStream = true;
              meta.warnings = [...meta.warnings,
                '已启用前台实时输出（Tee 捕获，日志：' + wrap.file + '）',
                ...(wrap.injectedPython ? ['已注入 PYTHONUNBUFFERED（避免 python 块缓冲）'] : [])];
              fgStreams.set('tool:' + String(exec.callId), { file: wrap.file, offset: 0 });
              diag('fgStream wrap', { tool: toolName, callId: exec && exec.callId, to: wrap.wrapped.slice(0, 160) });
            }
          } catch (e3) {
            diag('fgStream wrap FAILED', { tool: toolName, error: String(e3 && e3.message || e3) });
          }
        }
        // 前台且未启用实时输出时，才提示"完成后显示"
        if (!bg && meta.warnings.length && cfg.warn && !meta.fgStream) {
      ...
      if (r.fgStream) {
        ...
        if (f) {
          try {
            ...
            if (buf.length > f.offset) appendOutput(r, buf.slice(f.offset).toString('utf8'));
          } catch (e) { /* 文件未创建/已删：忽略 */ }
  ...
  // ---- 后台输出实时轮询（消耗性，受流开关控制）+ 前台 Tee 日志轮询 ----
  if (timer) {
    const offPoll = timer.interval(() => {
      // 前台 Tee 文件轮询（不受流开关影响：无 job_output 冲突）
      for (const r of records.values()) {
        if (r.kind !== 'tool' || !r.fgStream) continue;
        if (r.status !== 'running') continue;
        const f = fgStreams.get(r.key);
        if (!f) continue;
        try {
          const buf = readFileSync(f.file);
          if (buf.length > f.offset) {
            const text = buf.slice(f.offset).toString('utf8');
            f.offset = buf.length;
            appendOutput(r, text);
          }
