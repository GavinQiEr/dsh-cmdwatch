...
function apply(ctx) {
  ...
  const streamedCalls = new Set(); // 已完成流式推送的 callId（tools/result 跳过重复追加）
  let seq = 0;
  ...
  ctx.on('tools/execute', (exec, next) => {
    try {
      ...
      // shell.run 包装按 command 匹配认领，实现执行期间的增量推送
  ...
  ctx.on('tools/result', (exec, result) => {
    try {
      ...
      if (streamedCalls.has(String(exec.callId))) {
  ...
  // ---- 前台命令流式化：包装 shell.run ----
  // 机制：前台 pwsh/bash 的 shell.run 同步等待、中间输出不可见。我们把它替换为
  // start() + 增量 readOutput() 轮询，把执行中的输出实时推送到面板，同时在结束时
  // 组装与原生 run 相同的 ShellRunResult，保证工具行为（含模型侧结果）完全不变。
  if (shell && typeof shell.run === 'function') {
    const nativeRun = shell.run.bind(shell);
    shell.run = async (spec) => {
      // 1) 按 command 匹配待认领的前台流式候选（tools/execute 已登记）
      let matchedCallId = undefined;
      for (const [callId, cmd] of streamCandidates) {
        if (cmd === (spec && spec.command)) {
      ...
      if (matchedCallId === undefined) {
        // 非监控候选：完全走原生路径
        return nativeRun(spec);
      }
      streamCandidates.delete(matchedCallId);
      activeStreamCallId = matchedCallId;
      const target = records.get('tool:' + matchedCallId) || records.get('job:' + matchedCallId);
      if (!target || !shell.start || typeof shell.start !== 'function') return nativeRun(spec);

      // 2) 手动超时：shell.start 忽略 timeoutMs，需自行实现 deadline。
      //    start 内部把 signal 传给 spawnSpec，abort 会终止进程树（SIGTERM→grace→SIGKILL）。
      const controller = new AbortController();
      let timedOut = false;
      let timeoutTimer = undefined;
      const timeoutMs = spec && spec.timeoutMs ? spec.timeoutMs : 120000;
      const onUpstreamAbort = () => controller.abort();
      if (spec && spec.signal) {
        if (spec.signal.aborted) controller.abort();
        else spec.signal.addEventListener('abort', onUpstreamAbort, { once: true });
      }
      ...
      const cleanupTimer = () => {
        ...
        if (spec && spec.signal) spec.signal.removeEventListener('abort', onUpstreamAbort);
      };

      let proc;
      let offPoll;
      try {
        // 3) 以 start 方式启动，增量推送合并流（stdout+[stderr]）
        proc = shell.start({ ...spec, signal: controller.signal });
        const poll = () => {
          try {
            const read = proc.readOutput();
            const text = (read && read.delta) || '';
            if (text && target) appendOutput(target, text);
          } catch (e) { /* readOutput 可能因进程结束抛错 */ }
        };
        offPoll = timer && typeof timer.interval === 'function'
        ...
        // 4) 组装与原生 run 相同的结果结构。
        //    合并流整体作为 stdout（含 [stderr] 段，与后台 job_output 渲染一致），
        //    stderr 置空——pwsh 工具 renderPwshResult 会把它拼进模型可见文本。
        const read = proc.readOutput();
        const out = {
          text: (read && read.delta) || '',
          truncated: !!(read && read.lossy),
          ...(read && read.stdoutSpillPath ? { spillPath: read.stdoutSpillPath } : {})
        };
        const aborted = spec && spec.signal ? spec.signal.aborted : false;
        // 成功完成：标记已流式推送，tools/result 跳过重复追加
        ...
        return {
          ...
          // sandbox 事实由 sandbox.start 通过 onProcessDone 附加到 proc.sandbox；
          // 从那里恢复，保证与原 run 返回的 sandbox 段一致（denied/mode/enforcement）
          ...(proc.sandbox ? { sandbox: proc.sandbox } : {})
        };
      } catch (e) {
        cleanupTimer();
        // 基础设施失败：与原生 run 的拒绝语义一致。
        // 不标记 streamed，让 tools/result 正常追加错误文本。
        throw e;
      } finally {
        if (offPoll) offPoll();
