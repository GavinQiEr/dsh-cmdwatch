...
function apply(ctx, config) {
  ...
  ctx.on('tools/execute', (exec, next) => {
    try {
      ...
      // 诊断：后台调用无条件打日志（即使没有警示/改写），用于确认钩子是否触发
      if (bg) console.log(`[cmdmon] execute bg tool=${toolName} callId=${exec && exec.callId} cmd=${JSON.stringify((args && args.command || '').slice(0, 120))}`);
